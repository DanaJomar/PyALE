...
def order_groups(X, J):
    """
    Assign an order to the values of the feature J based on a distance matrix between
    the other variables/features in X
    """
    features = X.columns
    groups = X[J].cat.categories.values
    D_cumu = pd.DataFrame(0, index=groups, columns=groups)
    K = len(groups)
    for j in set(features) - set([J]):
        ...
        if (X[j].dtypes.name == 'category') | ((len(X[j].unique()) <= 10) & ('float' not in X[j].dtypes.name)):
            # counts and proportions of each value in j in each group in J (the studied feature)
            cross_counts = pd.crosstab(X[J], X[j])
